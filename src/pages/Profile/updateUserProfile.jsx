...
const UpdateUserProfile = () => {
   ...
   const sorcImag = 'http://localhost:3000/api/v1/image';
    ...
    const handleChange = (event) => {
        ...
        switch(name){
            case "user_name":
                setFormErrors({...formErrors,user_name:value.length>=3 && value.length <= 20?
                    null:"User Name must be at least 3 characters"
                })
                break;
            case "full_name":
              setFormErrors({...formErrors,full_name:value.length>=3 && value.length <= 20?
                null:"Full Name must be at least 3 characters"
               })
               break;
            case "phone":
                setFormErrors({...formErrors,phone:value.length===11?
                    null:"Phone  must be 11 number"
                });
                break;
            case "shop_name":
                setFormErrors({...formErrors,shop_name:value.length>=3?
                    null:"Shop Name must be at least 3 characters"
                });
                break;
            case "city":
                setFormErrors({...formErrors,city:value.length>=3?
                    null:"City must be at least 3 characters"
                });
                break;
            case "street":
                setFormErrors({...formErrors,street:value.length>=3?
...
const handleImageChange=(e)=>{
    ...
    //{`${sorcImag}${userData.profile_image}`} ||"Images/profileImage.png"
    return (
        <>
            <div className='container d-flex flex-column align-items-center DataContainer '>
                <form className="was-validated FormData  d-flex flex-column align-items-center col-12 col-6 ">
                    {/* //upload image */}
                    <Avatar className='imageProfile p-0' id='avatar' sx={{ width: 170, height: 140 }} alt="your Image" src={`${sorcImag}${imagesSrc}`}   />
